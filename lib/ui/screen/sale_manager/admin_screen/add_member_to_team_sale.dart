...
class AddMembertoTeamSaleScreen extends StatefulWidget {
  final int? saleTeamId;
  final String? saleTeamNameLeader;
  const AddMembertoTeamSaleScreen(
      {super.key, this.saleTeamId, this.saleTeamNameLeader});

  @override
  State<AddMembertoTeamSaleScreen> createState() =>
      _AddMemberToTeamSaleScreenState();
}

class _AddMemberToTeamSaleScreenState extends State<AddMembertoTeamSaleScreen> {
  ...
  Future<void> _addMemberToTeam() async {
    ...
    context.read<AddMemberToTeamSaleBloc>().add(
          AddMemberToTeamSale(
            saleTeamId: widget.saleTeamId ?? 0,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: TextApp(
          text: 'Team Leader ${widget.saleTeamNameLeader.toString()}',
          fontsize: 18.w,
                ...
                onPressed: _isLoading
                    ...
                    : () async {
                        showCustomDialogModal(
                            context: context,
                            textDesc: "Bạn có chắc muốn thêm member vào team ?",
                            title: "Thông báo",
      ...
      body: MultiBlocListener(
        listeners: [
          BlocListener<AddMemberToTeamSaleBloc, SaleManagerState>(
            listener: (context, state) {
              if (state is AddMemberToTeamSaleStateSuccess) {
                setState(() {
                  _isLoading = false;
                });
                showCustomDialogModal(
                    ...
                    isTwoButton: false);
              } else if (state is AddMemberToTeamSaleStateFailure) {
                setState(() {
                  _isLoading = false;
                });
                showCustomDialogModal(
